...
## 10. Install Zen Browser

Zen Browser isn't available as a Homebrew cask. Download the latest macOS `.dmg` from the [GitHub releases page](https://github.com/zen-browser/desktop/releases) and drag `Zen.app` into `/Applications`.

After installing, run `./apply.sh` (or `./scripts/02-symlinks.sh`) to copy `user.js`, containers, themes, and keyboard shortcuts into the active Zen profile.

If upgrading from an older "Zen Browser.app" (alpha) to "Zen.app" (release), the new app creates a separate profile. Copy profile data from the alpha profile to the release profile—see the Zen section of `02-symlinks.sh` for which files are managed.

## 11. Set default apps

macOS doesn't reliably allow setting default apps via the command line. Set these manually:

- **Default browser**: Zen — System Settings > Desktop & Dock > Default web browser
...
## 18. Internet Accounts & Apple Wallet
...
## 23. Set up scheduled sync/apply
...
## 24. Keychain prompts on secondary machines
...
## 25. Restore from Time Machine (optional)
